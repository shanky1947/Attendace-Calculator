...
{   
	...
	if(cs==1)
	  {
		...
		obj3.open("inc.dat",ios::binary|ios::out);
 		obj3.write((char *)&l,sizeof(l);
 		obj3.write((char *)&r,sizeof(r);
 		obj3.write((char *)&f,sizeof(f);
 		obj3.close();
		}
		else if(ch==3)
			studisplay();
	...
	/*	printf("Enter your registration no.");
		...
		scanf("%s",&ps);
		obj.open("stu.dat",ios::binary|ios::in);
		obj.seekg(431);
		obj.read((char *)&pswd,sizeof(pswd));
		printf("%s",pswd);
...
void endt()
{	
	obj.open("stu.dat",ios::binary|ios::out);
 	printf("Enter your registration no.            : ");
 	scanf("%s",&rg);
 	...
 	scanf("%s",&pswd);
 	obj.write((char *)&rg,sizeof(rg));
 	obj.write((char *)&nm,sizeof(nm));
 	obj.write((char *)&hb,sizeof(hb));
 	obj.write((char *)&br,sizeof(br));
 	obj.write((char *)&pswd,sizeof(pswd));
 	obj.close();
}
...
{	
	...
	char rs[50];
 	...
 	scanf("%s", &rs);
 	obj2.open("atd.dat",ios::binary|ios::in);
 	for(i=l;i>=0;i--)
 	{
 		obj2.seekg(i*sizeof(ad),ios::beg);
 		obj2.read((char *)&ad[i],sizeof(ad[i]));
...
void apush()
{
	...
	{	obj1.seekg(p*sizeof(t[f].sb[p]),ios::beg);
		obj1.read((char *)&t[f].sb[p],sizeof(t[f].sb[p]));
		...
		for(q=0;q<t[f].sr;q++)
		{
			if(strcmp(nm,ad[q].sbnm)==0)
			{
				...
				ad[q].a[ad[q].top]=na;
				obj2.write((char *)&ad[q],sizeof(ad[q]));
			}
		}
	}
}
 ...
 void enqueue()
 {
	...
	obj4.open("incc.dat",ios::binary|ios::app);
 	obj4.write((char *)&t[r].sr,sizeof(&t[r].sr));
 	obj4.close();
 }
 ...
 {	int sff;
 	...
 	{	sff=srr=0;
 		t[r].sf=t[r].sr=0;
 		obj1.open("sub.dat",ios::binary|ios::out);
 		...
 		obj1.write((char *)&t[r].sb[srr],sizeof(t[r].sb[srr]));
 		obj1.close();
 		if(r==0)
 		{
 		obj2.open("atd.dat",ios::binary|ios::out);
 		...
 		scanf("%d",&ad[l].tc);
 		obj2.write((char *)&ad[l],sizeof(ad[l]));
 		obj2.close();
 		}
 	}
	 else
	 {
	 	srr++;
	 	t[r].sr++;
	 	obj1.open("sub.dat",ios::binary|ios::app);
		printf("\nEnter details of %d subject: ",srr+1);
 		printf("\nEnter %d subject name: ",srr+1);
 		scanf("%s",&t[r].sb[srr].sn);
 		printf("Enter %d subject room no.: ",srr+1);
 		...
 		if(r==0)
 		{
 		obj2.open("atd.dat",ios::binary|ios::app);
 		strcpy(ad[l].sbnm,t[r].sb[srr].sn);
 		printf("Enter initial attendace of %d subject: ",srr+1);
 		scanf("%f",&ad[l].in);
 		printf("Enter total no. of classes till day of %d subject: ",srr+1);
...
void modify()
 {
 	int i,c=0,q,w;
 	f=0;
 	obj4.open("incc.dat",ios::binary|ios::in|ios::app);
 	obj4.read((char *)&,sizeof(t[0].sr));
 	char a[10];
 	obj1.open("sub.dat",ios::binary|ios::in);
 	w=t[f].sr;
 	for(i=0;i<=w;i++)
 	{obj1.seekg(i*sizeof(t[f].sb[i]));
	 obj1.read((char *)&t[f].sb[i],sizeof(t[f].sb[i]));	
	 q=t[f].sf;
...
{
	obj.open("stu.dat",ios::binary|ios::in);
	obj.read((char *)&rg,sizeof(rg));
	obj.read((char *)&nm,sizeof(nm));
	obj.read((char *)&hb,sizeof(hb));
	obj.read((char *)&br,sizeof(br));
...
void ttdisplay()
{
	...
	for(i=0;i<=r;i++)
	{
		...
		for(j=0;j<=t[i].sr;j++)
		{
		obj1.seekg(j*sizeof(t[i].sb[j]));
		obj1.read((char *)&t[i].sb[j],sizeof(t[i].sb[j]));
